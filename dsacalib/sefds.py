...
def get_autocorr_gains_off(msname, tvis, antenna_order, ant1, ant2, nspw, idxl0, idxr0, ant_transit_time, hwhms, nfint, npol):
    ...
    with table(f'{msname}.ms') as tb:
        for i, ant in enumerate(antenna_order):
            ...
            tidxs = np.concatenate((
                tidxl+np.arange(-10, 10),
                tidxr+np.arange(-10, 10)
            ))
...
def calculate_sefd(
    ...
    gain = np.nanmean(
        gain[:, :, idxl0:idxr0, :].reshape(nant, len(time), nfint, -1, npol),
        axis=3
    )
    ...
    for ifreq in range(nfint):
        ...
        for i in range(nant):
            for j in range(npol):
                ax[i].plot(
                    time-time[0], gain[i, :, ifreq, j], '.', color=ccyc[j]
                )
                ...
                try:
                    ...
                    params, cov = curve_fit(
                        _gauss, x[idx], y[idx], p0=initial_params
                    )
                ...
                eypred = np.sqrt(
                    ...
                    (err[2, 2]*params[0]*np.exp(earg)*xmb**2/params[2]**3)**2
                )
    ...
    if not repeat:
        apply_delay_bp_cal(
            msname,
            calname_delaycal,
            msnamecal=msname_delaycal
        )

    autocorr_gains_off, eautocorr_gains_off = get_autocorr_gains_off(
        msname, tvis, antenna_order, ant1, ant2, nspw, idxl0, idxr0,
        ant_transit_time, hwhms, nfint, npol
    )

    # Calculate the sefd
    sefds = (autocorr_gains_off/ant_gains-max_flux[:, np.newaxis, np.newaxis]/2)
    esefds = sefds*np.sqrt((eautocorr_gains_off/autocorr_gains_off)**2+(eant_gains/ant_gains)**2)
    ...
    return antenna_order+1, sefds, esefds, ant_gains, eant_gains, ant_transit_time, eant_transit_time, fref, hwhms, ehwhms
...
def plot_sefds(antennas, sefds, esefds, fref, ymax=None):
    ...
    for i in range(64):
        for j in range(2):
            ax[i//10].plot(fref, sefds[:, i, j], label=antennas[i] if j==0 else None, color=ccyc[i%10], ls=lcyc[j])
            ax[i//10].fill_between(fref, sefds[:, i, j]-esefds[:, i, j], sefds[:, i, j]+esefds[:, i, j], color=ccyc[i%10], alpha=0.3)
    for i in range(7):
        ax[i].legend()
